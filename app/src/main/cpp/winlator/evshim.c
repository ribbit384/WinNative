...
#define LOGI(...) __android_log_print(ANDROID_LOG_INFO, "EVSHIM", __VA_ARGS__)
#define LOGE(...) __android_log_print(ANDROID_LOG_ERROR, "EVSHIM", __VA_ARGS__)
#define LOGD(...)                                                              \
  do {                                                                         \
    if (g_debug_enabled)                                                       \
      __android_log_print(ANDROID_LOG_DEBUG, "EVSHIM", __VA_ARGS__);           \
  } while (0)
...
static void (*p_SDL_JoystickUpdate)(void);
...
static void *unified_updater(void *arg) {
  ...
  for (;;) {
    ...
    /* Flush SDL joystick events so winebus picks up axis changes immediately.
     * SDL_PumpEvents() processes the internal event queue - winebus needs this
     * to see events generated by virtual joystick updates.
     * SDL_JoystickUpdate() then delivers any pending joystick state changes
     * as SDL events that winebus can dequeue. */
    if (had_updates) {
      if (p_SDL_PumpEvents) p_SDL_PumpEvents();
      if (p_SDL_JoystickUpdate) p_SDL_JoystickUpdate();
    }
...
__attribute__((constructor)) static void initialize_all_pads(void) {
  ...
  GETFUNCPTR(SDL_JoystickUpdate);
  GETFUNCPTR(SDL_Delay);
  GETFUNCPTR(SDL_GetVersion);

  /* Load SDL_SetHint to configure hints before init */
  int (*p_SDL_SetHint)(const char *, const char *) = NULL;
  p_SDL_SetHint = (typeof(p_SDL_SetHint))dlsym(handle, "SDL_SetHint");

  /* Critical: allow joystick events even when window doesn't have focus.
   * In Wine's multi-process architecture, the winebus process (which reads
   * SDL joystick events) doesn't own any visible window, so without this
   * hint, SDL silently drops all joystick events. */
  if (p_SDL_SetHint) {
    p_SDL_SetHint("SDL_JOYSTICK_ALLOW_BACKGROUND_EVENTS", "1");
    LOGI("SDL hint SDL_JOYSTICK_ALLOW_BACKGROUND_EVENTS set\n");
  }
